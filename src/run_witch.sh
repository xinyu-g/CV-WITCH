...
# Function to clean up file contents
process_file() {
    local file="$1"
    # Replace odd characters in file contents, preserving '>' character
    # Create a temporary file
    sed 's/[^a-zA-Z0-9>]/_/g' "$file" > "${file}.tmp"
    mv "${file}.tmp" "$file"
}

# Create output directory if it doesn't exist
mkdir -p "$PROJECT_ROOT/witch_output"
...
process_directory() {
    ...
    for file in "${dir_files[@]}"; do
        if [ -f "$file" ]; then
            ...
            output_dir="$PROJECT_ROOT/witch_output/$output_prefix/$dir_path/$filename_no_ext"
            
            # Check if the directory exists and has completed alignment
            if [ -f "$output_dir/aligned.fasta" ] && [ -s "$output_dir/aligned.fasta" ]; then
            ...
            # Run WITCH
            echo "Running WITCH on $file..."
            witch-msa -i "$file" -d "$output_dir"
            
            # Check if the run was successful and aligned.fasta exists and is not empty
            if [ $? -eq 0 ] && [ -f "$output_dir/aligned.fasta" ] && [ -s "$output_dir/aligned.fasta" ]; then
                echo "Successfully completed WITCH for $filename"
            else
                echo "Error processing $filename or output file not found/empty"
            fi
